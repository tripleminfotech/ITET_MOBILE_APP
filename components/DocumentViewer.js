...
export default class DocumentView extends React.Component {
        ...
        RNFetchBlob.fs.exists(options.path)
            .then((exist) => {
              ...
              if(exist){
                ...
                RNFetchBlob.android.actionViewIntent(options.path,mime.lookup(options.path)).then({}).catch((err)=>{console.log(err)})
              }
              else{
                  this.setState({
                      downloading:true
                ...
                config(options)
                  ...
                  .then(res => {
                    ...
                    RNFetchBlob.android.actionViewIntent(res.path(),mime.lookup(options.path))
    ...
    render(){
        return (
            ...
            <TouchableOpacity onPress={()=>this.checkPermission()} style={{margin:10,padding:10,borderRadius:15,backgroundColor:"white",flexDirection:"row",alignItems:"center"}}>

...
const PopularCourse = ({item, onPress, click }) => {
  ...
  const rating = parseFloat(item.course_rating).toFixed(1);
    return(
            ...
            <TouchableOpacity onPress={() => onPress(item.id)} style={{ marginLeft: 20, marginBottom:15,backgroundColor:"#F4F4F4",borderRadius:10,flexDirection:"row",alignItems:"center",justifyContent:"space-between",padding:15}}>
              ...
              <View style={{marginLeft:10,width:180}}>
                ...
                <Text style={{color:"#C5CEE0",fontSize:13}}>({item.total_rating} reviews)</Text>
                </View>
              </View>

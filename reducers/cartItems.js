...
const cartItems = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case 'ADD_TO_CART':
            AsyncStorage.setItem('cart',JSON.stringify([...state.product, action.payload]));
            return  {
        ...
        case 'REMOVE_FROM_CART':
            AsyncStorage.setItem('cart',JSON.stringify(state.product.filter(cartItem => cartItem.id !== action.payload.id)));
            return {
                product:state.product.filter(cartItem => cartItem.id !== action.payload.id),
                total:getPrice(state.product.filter(cartItem => cartItem.id !== action.payload.id))
            }
        case 'EMPTY_CART':
    ...
    switch (action.type) {
        ...
        case 'UPDATE_READ':
            ...
            for(i=0;i<state.notification.length;i++){
                if(state.notification[i].id === action.payload && !state.notification[i].read){

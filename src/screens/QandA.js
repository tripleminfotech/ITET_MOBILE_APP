  ...
  const renderQuestions = (item) => {
    return (
      <View
        ...
        {item.audio_attachment !== null ? (
          <AudioPlayer url={item.audio_attachment} />
        ) : null}
        {item.attachment_url !== null ? (
          <DocumentView url={item.attachment_url} />
        ) : null}
        <Text style={{marginTop: 5, color: '#821A1A'}}>
          {item.answer.length > 0 ? item.answer.length : 'no'}responses
        </Text>
        <FlatList
          data={item.answer}

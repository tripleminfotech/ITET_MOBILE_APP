...
export default function Search({navigation}){
    ...
    return (
        ...
        <Animate.View
            ...
            duration={1000}
         style={{flexDirection:"row",justifyContent:"center",alignItems: 'center',backgroundColor:"white",margin:20,borderRadius:8,shadowOffset: {
          width: 0,
          height: 11,
      },
      shadowOpacity: 0.57,
      ...
      elevation: 3}}>
         ...
         <MaterialIcon style={styles.searchIcon} name="search" size={20} color="#000"/>}
            <TextInput 
                ...
                />
            {focus &&  <MaterialIcon name="clear" size={20} style={{marginRight:20}} onPress={()=>{setFocus(false);setSearch('');setSubmit(false);setError(false)}}/>}
        </Animate.View>
        </View>
         <View style={{flex: 1,backgroundColor:"white"}}>
         {isLoading?
         <FlatList
          ...
          />
         :isTyping?<Text style={{textAlign:"center"}}>click search to get result for "{search}"</Text>:submit?<FlatList 
                    data = {searchData}
                    renderItem={({item})=><CourseComponent item={item} onPress={onPress} click={onPresss} />}
                    keyExtractor={item=> item.id}
                    /> :error?
                 <View style={{flex:1,justifyContent:"center",alignItems:"center"}}>
                 <Text style={{textAlign:"center",fontSize:35}}>data not found</Text>
                 </View>:
                 <View>
                   <Text style={{marginLeft:20,fontSize:17,marginBottom:10}}>Maybe you like</Text>
                   <FlatList 
                   showsVerticalScrollIndicator = {false}
                    data = {data}
                    style={{marginBottom:30,marginRight:20}}
                    renderItem={({item})=><PopularCourse item={item} onPress={onPress} click={onPresss} />}
                    keyExtractor={item=> item.id}
                    />
                 </View>
                 }

...
export default class Test extends React.Component {
    ...
    _submitTest = async() => {
        ...
        for (var i in json) {
            if (json[i].selected !== '') {
                if (
                    (json[i].selected + 1).toString() ===
                    JSON.parse(json[i].correct_answers)[0]
                ) {
                    count = count + 1;
                }
                check = true;
    ...
    _renderItem = ({ item }) => {
        ...
        return ( <
            ...
            /View> <
            FlatList data = { JSON.parse(item.options) }
            keyExtractor = {
                (item) => item }
            renderItem = {
                ({ index, item }) => ( <
                    ...
                    CheckBox style = {
                        { marginRight: 20 } }
                    checked = { select.selected === index }
                    color = "#fc5185"
                    onPress = {
                        () => this._onPress({ id, index }) }
                    /> <
                    /View>
                )
            }
    ...
    render() {
        return ( <
            ...
            Text style = {
                { marginBottom: 10 } } >
            Your Current Mark - { this.state.current_mark } <
            /Text> <
            ...
            <
            Text style = {
                { textAlign: 'center', color: 'white' } } > DONE < /Text> <
            /TouchableOpacity> <
            /View> <
            /Modal> {
                this.state.attempt !== 3 ? ( <
                    ...
                    Your Highest Mark is { this.state.mark } <
                    /Text> <
                    TouchableOpacity onPress = {
                        () => this.props.navigatio.goBack() } >
                    <
                    Text style = {
                        { color: appColor } } > Click Here To Goback < /Text> <
                    /TouchableOpacity> <
                    /View>
                )

...
function VideoPlayer({navigatio, data, completed, next, rout}) {
  ...
  const SectionHead = ({title}) => {
    return (
      <Text style={{marginLeft: 10, marginBottom: 10}}>Section {title}</Text>
    );
  };
  const SectionItem = ({item, index}) => {
    return (
      <TouchableOpacity
        onPress={() => onNextprev(item.id)}
        style={{
          padding: 8,
          backgroundColor: item.id === data.id ? '#f1f1f1' : 'white',
        }}>
        <View
  ...
  const renderCollapse = () => {
    const arr = bookmark.map((item) => (
      <View key={item.id} style={styles.collapsibleItem}>
        ...
        <TouchableOpacity
          onPress={() => (
            videoRef.current.seek(item.duration),
            setState({...state, play: true})
          )}>
          <Text style={{color: 'green'}}>
            {' '}
            {getMinutesFromSeconds(item.duration)}{' '}
          </Text>
        </TouchableOpacity>
      </View>
    ));
  ...
  const submitBookmark = async () => {
    ...
    await Api(`/bookmark/create/`, 'POST', {
      lessonId: data.id,
      notes: value,
    ...
    setBookmark([
      ...bookmark,
      {id: data.id, note: value, duration: state.currentTime},
    ]);
    setValue('');
    onClose();
  ...
  const fetchVideo = async () => {
    ...
    setBookmark(
      data.bookmarks.length > 0
        ? JSON.parse(data.bookmarks[0].notes)
        : bookmark,
    );
  };
      ...
      {isLoading ? (
        ...
        <View style={styles.container}>
              ...
              {state.showControls && (
                <View style={styles.controlOverlay}>
                  ...
                  <PlayerControls
                    ...
                    previousDisabled={
                      data.previous_lesson !== null ? false : true
                    }
          ...
          <Text style={{fontSize:20}}>{data.title}</Text>
              ...
              <View
                ...
                <TouchableOpacity
                  onPress={() => onOpen()}
                  style={{padding: 9, backgroundColor: '#0095FF'}}>
                ...
                {data.is_completed === 1 ? (
                  <TouchableOpacity
                    disabled={true}
                    style={{padding: 9, backgroundColor: '#32a852'}}>
                    <Text style={{color: 'white'}}>completed</Text>
                  </TouchableOpacity>
                ) : (
                  <TouchableOpacity
                    onPress={() => completed()}
                    style={{padding: 9, backgroundColor: '#FF0000'}}>
        ...
        <View>
          ...
          {renderSection()}
              ...
              {index === 1 && <View>{renderCollapse()}</View>}
            </>
          )}
        </View>
      )}
